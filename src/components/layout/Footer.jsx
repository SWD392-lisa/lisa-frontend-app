import React from 'react';
import { Link } from 'react-router-dom';
import './Footer.css';

// SVG Icons for Socials (Safe, no third-party dependencies)
const SpotifyIcon = () => (
  <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.54.659.3 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15.001 10.62 18.66 12.84c.361.181.54.78.3 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.781s.18-1.2.78-1.381c4.26-1.26 11.28-1.02 15.721 1.621.54.3.72 1.02.42 1.56-.24.54-.96.72-1.62.36z"/></svg>
);

const FacebookIcon = () => (
  <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
);

const InstagramIcon = () => (
  <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
);

const YouTubeIcon = () => (
  <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
);

const XIcon = () => (
  <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z"/></svg>
);

const DownloadAppIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
    <polyline points="7 10 12 15 17 10" />
    <line x1="12" y1="15" x2="12" y2="3" />
  </svg>
);

export const Footer = () => {
  return (
    <>
      <footer className="global-footer">
        <div className="footer-container">
          
          {/* 1. Top Section: Link Columns */}
          <div className="footer-grid">
            <div className="footer-col">
              <h3>About LUCY</h3>
              <Link to="/about">Introduction</Link>
              <Link to="/about/vision">Vision</Link>
              <Link to="/about/team">Team</Link>
              <Link to="/contact">Contact Us</Link>
            </div>
            
            <div className="footer-col">
              <h3>Discover</h3>
              <Link to="/courses">100-Level Path</Link>
              <Link to="/discover/mentors">Featured Mentors</Link>
              <Link to="/events/podcasts">Exclusive Podcasts</Link>
            </div>
            
            <div className="footer-col">
              <h3>Student Support</h3>
              <Link to="/support">Help Center</Link>
              <Link to="/support/guidelines">Community Guidelines</Link>
              <Link to="/support/report">Report a Violation</Link>
            </div>
            
            <div className="footer-col">
              <h3>Partners & Events</h3>
              <Link to="/mentors/apply">For Mentors</Link>
              <Link to="/events">Upcoming Events</Link>
              <Link to="/app">Download App</Link>
            </div>
          </div>

          {/* 2. Divider */}
          <hr className="footer-divider" />

          {/* 3. Bottom Section: Socials, Legal, Copyright */}
          <div className="footer-bottom">
            <div className="footer-socials">
              <a href="#" className="social-circle-btn" aria-label="Spotify">
                <SpotifyIcon />
              </a>
              <a href="#" className="social-circle-btn" aria-label="Facebook">
                <FacebookIcon />
              </a>
              <a href="#" className="social-circle-btn" aria-label="Instagram">
                <InstagramIcon />
              </a>
              <a href="#" className="social-circle-btn" aria-label="YouTube">
                <YouTubeIcon />
              </a>
              <a href="#" className="social-circle-btn" aria-label="X (Twitter)">
                <XIcon />
              </a>
            </div>

            <div className="footer-legal-list">
              <Link to="/legal/privacy">Identity Security Policy</Link>
              <span className="footer-legal-separator hidden-mobile"></span>
              <Link to="/legal/terms">Terms of Use</Link>
              <span className="footer-legal-separator hidden-mobile"></span>
              <Link to="/legal/data">Data Privacy</Link>
              <span className="footer-legal-separator hidden-mobile"></span>
              <Link to="/legal/cookies">Cookie Preferences</Link>
            </div>

            <p className="footer-copyright">
              © 2026 LUCY EdTech. All rights reserved.
            </p>
          </div>
        </div>
      </footer>

    </>
  );
};
