const configuredLmsUrl = import.meta.env.VITE_LMS_API_URL || 'http://localhost:8080/api/lms';
const LMS_BASE_URL = configuredLmsUrl.replace(/\/$/, '');
const CURRICULUM_BASE_URL = LMS_BASE_URL.replace(/\/api\/lms$/, '/api/curriculum');
const REALTIME_BASE_URL = (import.meta.env.VITE_REALTIME_URL || 'http://localhost:3001').replace(/\/$/, '');
